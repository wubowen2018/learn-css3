...
2. 规则：浮动元素产生了浮动流，所有产生了浮动流的元素，块级元素就看不到他们，但是`产生了bfc的元素` 和 `带有文本类属性（inline-block）的元素`，以及`文本元素`依然可以看到他们（浮动元素）。`看不到他们，会当他们不存在，接着渲染`。
...
`正确的解决方案`
   ...
   2. 把父元素变成bfc元素或者变成带有文本属性（inline- block）的元素，变成bfc元素的条件：`position:absolute;`,`overflow:hidden;`,`float:left/right`,`display:inline-block;`

>`注意：设置了position: absoulate / float : left / right 的元素会在系统内部变成inline-block `
...
border-width : 粗细 像素值 / 相对单位
border-style : solid dotted dashed  
border-corlor: 颜色的表示方法

/*也可以分别设置*/
border-left-color:
border-right/top/bottom: 2px solid
```
`关于值的个数与对应关系，适用于margin，padding，border-width` 
...
|阴影颜色| 可选。 默认(黑色) |
|投影方式| 外部阴影-outset(默认)   内部阴影-inset |
|||
...
### 2-3、边框背景图片 -- 几乎不用
```css
border-image:url(borderimg.png) 70 70 70 70 repeat;/* url的参数为图片路径字符串 ，70 70 70 70 遵循顺时针原则，上右下左，单位是像素，也支持百分比  延伸方式由三种 repeat - 重复 round - 平铺 stretch - 拉伸*/
```
...
2. 单词 red blue orange gray yellow 
3. #+16进制数字  #333333
...
```
`linear-gradient()函数的参数除了用单词表示，还可以用角度，对应关系如下：`
...
`注意：`并列选择器，当标签和class并列的时候，class写在后面
